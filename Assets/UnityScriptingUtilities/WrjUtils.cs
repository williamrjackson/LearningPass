    ...
    public class Utils : MonoBehaviour
    {
        ...
        public class MapToCurve
        {
            ...
            public Manipulation ChangeColor(Transform tform, Color to, float duration, bool mirrorCurve = false, int loop = 0, int pingPong = 0, int mirrorPingPong = 0, bool useTimeScale = false, OnDone onDone = null)
            {
                ...
                else if (tform.GetComponent<Renderer>())
                {
                ...
                else if (tform.GetComponent<TMPro.TMP_Text>())
                {
                    mcp.coroutine = UtilObject().StartCoroutine(LerpTmpColor(mcp, tform, to, duration, mirrorCurve, loop, pingPong, mirrorPingPong, useTimeScale, onDone));
                }
                else
                {
                    Debug.LogWarning("MapToCurve.ChangeColor not implemented for objects like " + tform.name + ". \n GameObject requires a UI.Image or Renderer Component.");
                    return null;
                }
                UtilObject().AddToCoroList(mcp);
                return mcp;
            ...
            private IEnumerator LerpTmpColor(Manipulation mcp, Transform tform, Color to, float duration, bool mirrorCurve, int loop, int pingPong, int mirrorPingPong, bool useTimeScale, OnDone onDone)
            {
                float elapsedTime = 0;
                mcp.iterationCount++;
                TMPro.TMP_Text text = tform.GetComponent<TMPro.TMP_Text>();
